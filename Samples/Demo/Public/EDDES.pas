...
type
  ...
  function EncryHex(PData : array of Byte;KeyData :array of Byte;var EData : array of Byte) : Boolean;
  function DecryHex(PData : array of Byte;KeyData :array of Byte;var EData : array of Byte) : Boolean;
...
implementation
function InverseHEXStr(HEXStr : ansistring) : ansistring;
var
  vStr : ansistring;
  I : Integer;
begin
  vStr := '';
  while (Length(HEXStr) mod 2) <> 0 do
    HEXStr := HEXStr +'0';
  for I := 0 to (Length(HEXStr) div 2) -1 do
    vStr := vStr + IntToHex(255-StrtoInt('$'+Copy(HEXStr,I*2+1,2)),2);
  Result := vStr;
end;
...
begin
  ...
  for I := 0 to Length(PData) div 8 - 1 do
  begin
    CopyMemory(@StrByte[0],@PData[I*8],8);
    desData(dmEncry, StrByte, OutByte);
    CopyMemory(@EData[I*8],@OutByte[0],8);
  end;
  Result := True;
end;
