...
uses
  SysUtils, Classes, Windows,System.SyncObjs;
...
type
  ...
  public
    ...
    property Locking : Boolean read FLocking;
    constructor Create;
    ...
    procedure GetKeys(Keys : TStringList);overload;
    procedure GetKeys(beginning : string;Keys : TStringList);overload;
    function RemoveAll : Boolean;
    procedure LockHash;
    procedure UnLockHash;
  end;
...
begin
  try
    ...
    while Assigned(he) do
    begin
      if he^.Hash=h then
        if he^.key=_key then
        begin //gefunden (English: found)
          Result:=True;
          Exit;
        end;
      he:=he^.next;
...
procedure THash.GetKeys(beginning: string; Keys: TStringList);
var
  i: Integer;
  he: PHashEntry;
begin
...
procedure THash.GetKeys(Keys: TStringList);
var
  i: Integer;
  he: PHashEntry;
begin
...
begin
  ...
  if (newsize>MaxCapacity) and (MaxCapacity<>-1) then
  begin
    Exit;
  end;
...
function THash.RemoveAll: Boolean;
var
  i : Integer;
  vList : TStringList;
begin
